        ...
        match &self.stmt {
            StmtEnum::ReturnStmt(number) => {
                ...
                let return_value = match number.number {
                    NumberEnum::IntConst(int_const) => curr_func_data.dfg_mut().new_value().integer(int_const), //如果匹配，则解构出int_const,构造返回值
                    _ => return Err("Unsupported number variant"), //此时的NumberEnum中只有Int型
                };
            ...
            _ => return Err("Unsupported statement variant"),  //此时的StmtEnum中只有RETURN
        }
        Ok(())
    }
